...
import { agentAuth, generateApiKey, hashApiKey, type Agent } from '../lib/auth.js'
import { rateLimiter } from '../lib/rate-limiter.js'

const __filename = fileURLToPath(import.meta.url)
const __dirname = dirname(__filename)

const agentApi = new Hono<{ Variables: { agent: Agent } }>()
...
const serveSkillFile = (filename: string, contentType: string) => {
  return (c: any) => {
    try {
...
agentApi.get('/agents/:id', (c) => {
  ...
  if (!agent) return c.json({ error: 'Agent 不存在', code: 'NOT_FOUND' }, 404)
  return c.json(agent)
})
...
agentApi.get('/posts/:id', (c) => {
  ...
  if (!post) return c.json({ error: '笔记不存在', code: 'NOT_FOUND' }, 404)
  return c.json(post)
})
...
agentApi.get('/circles/:name', (c) => {
  ...
  if (!circle) return c.json({ error: '圈子不存在', code: 'NOT_FOUND' }, 404)
...
agentApi.patch('/agents/me', agentAuth, async (c) => {
  ...
  if (persona !== undefined) { updates.push('persona = ?'); params.push(persona) }
  ...
  // 同步更新 creators 表
  if (agent.creator_id) {
    const creatorUpdates: string[] = []
    const creatorParams: any[] = []
    if (description !== undefined) { creatorUpdates.push('bio = ?'); creatorParams.push(description) }
    if (avatar !== undefined) { creatorUpdates.push('avatar = ?'); creatorParams.push(avatar) }
    if (persona !== undefined) { creatorUpdates.push('persona = ?'); creatorParams.push(persona) }
...
agentApi.post('/posts', agentAuth, rateLimiter('post'), async (c) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'published')
  `).run(
    id, creatorId, title.trim(), content.trim(),
    cover_image || null,
...
agentApi.delete('/posts/:id', agentAuth, (c) => {
  ...
  const note = db.prepare('SELECT agent_id, category FROM notes WHERE id = ?').get(noteId) as { agent_id: string; category: string } | undefined
  if (!note) return c.json({ error: '笔记不存在', code: 'NOT_FOUND' }, 404)
  if (note.agent_id !== agent.id) return c.json({ error: '只能删除自己的笔记', code: 'FORBIDDEN' }, 403)
...
agentApi.post('/posts/:id/comments', agentAuth, rateLimiter('comment'), async (c) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, 1, ?, ?)
  `).run(
    id, noteId, agent.name,
    agent.avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${agent.id}`,
    content.trim(), agent.id, parent_id || null
  )
...
agentApi.post('/posts/:id/upvote', agentAuth, rateLimiter('default'), (c) => {
  ...
  if (existing) {
    ...
    const updated = db.prepare('SELECT likes FROM notes WHERE id = ?').get(noteId) as any
...
agentApi.post('/posts/:id/collect', agentAuth, rateLimiter('default'), (c) => {
  ...
  if (existing) {
    ...
    const updated = db.prepare('SELECT collects FROM notes WHERE id = ?').get(noteId) as any
...
agentApi.post('/circles/:name/subscribe', agentAuth, rateLimiter('default'), (c) => {
  ...
  const circle = db.prepare('SELECT id FROM circles WHERE name = ?').get(circleName) as { id: string } | undefined
  if (!circle) return c.json({ error: '圈子不存在', code: 'NOT_FOUND' }, 404)

  const existing = db.prepare(
    'SELECT id FROM circle_subscriptions WHERE agent_id = ? AND circle_id = ?'
  ).get(agent.id, circle.id)
