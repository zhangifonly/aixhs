...
import { matchCategory } from './topic-matcher.js'
...
const MOCK_HOT_TOPICS: Record<string, string[]> = {
  beauty: [
    '黄黑皮逆袭冷白皮的秘密',
    '烂脸急救指南亲测有效',
    '毛孔粗大这样做真的会变小',
 ...
 * 获取模拟热点话题
 */
function getMockHotTopics(): { title: string; category: string; heat_score: number }[] {
  const topics: { title: string; category: string; heat_score: number }[] = []
 ...
 * 实际生产环境需要替换为真实爬虫
 */
export async function crawlHotTopics(): Promise<number> {
 ...
 * 获取待处理的热点话题
 */
export function getPendingTopics(limit: number = 10): HotTopic[] {
  return db.prepare(`
    SELECT * FROM hot_topics
    WHERE status = 'pending'
    ORDER BY heat_score DESC, created_at DESC
    LIMIT ?
  `).all(limit) as HotTopic[]
}
...
export function getHotTopics(options?: {
  ...
  return db.prepare(sql).all(...params) as HotTopic[]
}

/**
 * 获取单个热点话题
 */
export function getHotTopic(id: string): HotTopic | null {
  return db.prepare('SELECT * FROM hot_topics WHERE id = ?').get(id) as HotTopic | null
}

/**
 * 更新热点话题状态
 */
export function updateHotTopicStatus(
  id: string,
  status: HotTopic['status'],
  noteId?: string,
  errorMessage?: string
): void {
 ...
 * 清理过期热点（超过7天）
 */
export function cleanExpiredTopics(): number {
