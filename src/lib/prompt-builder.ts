...
export interface StyleConfig {
  ...
  writingPrinciples: Record<string, string>
  antiAIDetection: {
    description: string
    sentenceVariation: Record<string, any>
    pronounLimits: Record<string, any>
    forbiddenPatterns: { pattern: string; reason: string; alternative: string }[]
    encouragedPatterns: { pattern: string; example: string; effect: string }[]
  }
  titleRules: Record<string, any>
  contentStructure: Record<string, any>
  tagRules: Record<string, any>
...
export interface ReferenceArticle {
  ...
  quality: string
  keyFeatures: string
  metrics: { likes: number; collects: number }
  tags: string[]
}
...
export function loadStyleConfig(): StyleConfig {
  if (styleConfig) return styleConfig
 ...
 * 加载参考文章
 */
export function loadReferences(categoryId: string): ReferenceArticle[] {
 ...
 * 构建读者心理分析部分
 */
function buildReaderPsychologySection(profile: CategoryProfile): string {
 ...
 * 构建写作策略部分
 */
function buildWritingStrategySection(profile: CategoryProfile, subTopic?: SubTopic): string {
 ...
 * 构建反AI检测规范
 */
function buildAntiAISection(): string {
 ...
 * 构建参考文章部分（Few-shot）
 */
function buildReferencesSection(references: ReferenceArticle[]): string {
 ...
 * 构建写作禁忌部分
 */
function buildTaboosSection(profile: CategoryProfile): string {
  return `【写作禁忌】
 ...
 * 构建完整的写作提示词
 */
export function buildSmartPrompt(options: PromptBuildOptions): string {
  const {
    creator,
    topic,
    categoryId,
 ...
 * 简单提示词（降级方案）
 */
function buildSimplePrompt(creator: Creator, topic: string): string {
 ...
 * 获取板块的示例标题
 */
export function getExampleTitles(categoryId: string): string[] {
 ...
 * 获取当季热门话题建议
 */
export function getSeasonalSuggestions(categoryId: string): string[] {
  const profile = getCategoryProfile(categoryId)
  if (!profile) return []
