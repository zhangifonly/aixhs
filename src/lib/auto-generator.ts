...
import {
  getPendingTopics,
  updateHotTopicStatus,
  type HotTopic
} from './hot-topic-crawler.js'
import {
  generateNoteStream,
  parseNoteContent,
  saveNote,
 ...
 * 为笔记生成封面图
 */
async function generateCoverImage(noteId: string, title: string, category: string): Promise<void> {
 ...
 * 获取分类对应的博主
 */
function getCreatorByCategory(category: string): Creator | null {
  if (!creatorsCache) {
    creatorsCache = loadCreators()
  }

  // 分类到博主ID的映射
  const categoryToCreator: Record<string, string> = {
    '美妆护肤': 'xiaomei',
    '穿搭时尚': 'chuanda',
    '美食探店': 'chihuo',
    '旅行攻略': 'lvxing',
 ...
 * 生成单个热点话题的笔记
 */
export async function generateNoteForTopic(topic: HotTopic): Promise<string | null> {
 ...
 * 批量处理待生成的热点话题
 */
export async function processPendingTopics(limit: number = 5): Promise<{
  success: number
  failed: number
  noteIds: string[]
}> {
