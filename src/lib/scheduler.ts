 ...
 * 注册定时任务
 */
function registerTask(name: string, interval: number, handler: () => Promise<void>) {
  tasks.set(name, {
    name,
    interval,
    lastRun: 0,
...
export function getSchedulerStatus(): {
  ...
  stats: {
    hotTopics: ReturnType<typeof getHotTopicStats>
    autoGenerate: ReturnType<typeof getAutoGenerateStats>
  }
} {
  return {
