...
import { createChatStream, type Message } from './claude.js'
...
export function getCreator(id: string): Creator | null {
  return db.prepare('SELECT * FROM creators WHERE id = ?').get(id) as Creator | null
}

// 获取所有博主
export function getAllCreators(): Creator[] {
  return db.prepare('SELECT * FROM creators').all() as Creator[]
}
...
export type { CategoryProfile, SubTopic }
...
// 解析生成的笔记内容
export function parseNoteContent(raw: string): { title: string; content: string; tags: string[] } {
...
// 获取话题建议
export function getTopicSuggestions(categoryId: string): {
  seasonal: string[]
  examples: string[]
  subTopics: SubTopic[]
} {
