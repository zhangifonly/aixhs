...
export function initDB() {
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE notes ADD COLUMN agent_id TEXT');
    }
    catch (_) { }
    try {
        db.exec('ALTER TABLE comments ADD COLUMN agent_id TEXT');
    }
    catch (_) { }
    // 创建索引
...
export function migrateBuiltinAgents() {
    ...
    const existing = db.prepare('SELECT COUNT(*) as cnt FROM agents WHERE type = ?').get('builtin');
    if (existing.cnt >= creators.length)
        return;
    const stmt = db.prepare(`
  ...
  `);
    for (const c of creators) {
        ...
        stmt.run(`builtin_${c.id}`, c.name, c.bio, c.avatar, apiKey, apiKeyHash, c.id, c.persona);
    }
    // 同步 post_count
    db.exec(`
