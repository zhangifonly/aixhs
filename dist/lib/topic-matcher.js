...
// 缓存
let categoryProfiles = null;
/**
 * 加载板块画像配置
 */
export function loadCategoryProfiles() {
    if (categoryProfiles)
 ...
 * 计算话题与关键词的匹配分数
 */
function calculateMatchScore(topic, keywords) {
 ...
 * 匹配最相关的板块
 */
export function matchCategory(topic) {
 ...
 * 匹配最相关的细分话题
 */
export function matchSubTopic(topic, categoryId) {
    const profiles = loadCategoryProfiles();
    let bestMatch = null;
    let bestScore = 0;
 ...
 * 获取板块的当季热门话题
 */
export function getSeasonalTopics(categoryId) {
    const profiles = loadCategoryProfiles();
 ...
 * 获取板块的所有细分话题
 */
export function getSubTopics(categoryId) {
    const profiles = loadCategoryProfiles();
    const profile = profiles.find(p => p.id === categoryId);
